...
def perceptron_sgd_function(X, Y):
    ...
    eta = 1#constant
    epochs = 20#how many time you gradient descent
    ...
    for t in range(epochs):
        for i, x in enumerate(X):
            ...
            print((np.dot(X[i], w)*Y[i]))#sum product(x*w) S(predict)*Y(actual) is oppposite then error 
            if (np.dot(X[i], w)*Y[i]) <= 0:
                print(np.dot(X[i], w)*Y[i])
                print("Negative product for: ..")
                print(X[i], w, Y[i])
                w = w + eta*X[i]*Y[i] #this is tarining code
